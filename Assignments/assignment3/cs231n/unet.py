...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        x = (cfg_scale + 1)*self.forward(x, time, model_kwargs) - cfg_scale*self.forward(x, time, {'text_emb' : None})
        ##################################################################
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for module in self.downs:
          for i, block in enumerate(module):
            x = block(x, context) if i < 2 else block(x)
            if i < 2:
              down_outs.append(x)
        # (2)
        ...
        for i, module in enumerate(self.ups):
          for j, block in enumerate(module):
            x = block(torch.cat((x, down_outs[len(down_outs) - i*2 - j]), dim=1), context) if j > 0 else block(x)
        ##################################################################
