...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        i_masks = torch.arange(0, max_len).repeat(embed_dim, 1).T
        exp_masks = torch.tensor([(k//2)*2 for k in range(embed_dim)]).repeat(max_len, 1) / embed_dim
        even_j_masks = torch.tensor([1, 0]).repeat(max_len, embed_dim//2)

        pe1 = torch.sin(i_masks*(10000**(-exp_masks))) * even_j_masks
        pe2 = torch.cos(i_masks*(10000**(-exp_masks))) * (1 - even_j_masks)
        pe = (pe1 + pe2).unsqueeze(0)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        q = torch.moveaxis(q.reshape(N, S, self.n_head, self.head_dim), 2, 1)
        k = torch.moveaxis(k.reshape(N, T, self.n_head, self.head_dim), 2, 1)
        v = torch.moveaxis(v.reshape(N, T, self.n_head, self.head_dim), 2, 1)

        a = (torch.matmul(q, torch.transpose(k, -2, -1)))/(self.head_dim**0.5) # (N, H, S, T)
        
        if attn_mask is not None:
          a = torch.masked_fill(input=a, mask=(~attn_mask).unsqueeze(0).unsqueeze(0).expand(N, self.n_head, S, T), value=float('-inf'))
        
        output = self.attn_drop(torch.softmax(a, dim=-1)) @ v # (N, H, S, E\\H)

        output = torch.moveaxis(output, 2, 1).reshape(N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # (N, num_patch_h, num_patch_w, C, patch_size, patch_size)
        x = x.flatten(1, 2)  # (N, num_patches, C, patch_size, patch_size)
        x = x.flatten(2)     # (N, num_patches, patch_dim)
        out = self.proj(x)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
