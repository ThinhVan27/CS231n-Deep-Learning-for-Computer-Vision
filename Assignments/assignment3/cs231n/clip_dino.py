...
def get_similarity_no_loop(text_features, image_features):
    ...
    (torch.linalg.norm(text_features, dim=-1).unsqueeze(1) * torch.linalg.norm(image_features, dim=-1).unsqueeze(0))
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    # Predict
    pred_classes = [class_texts[i] for i in list(torch.argmax(sim, dim=0))]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        images_tensor = torch.cat(processed_images, dim=0).to(device)
        self.images_features = clip_model.encode_image(images_tensor)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        ############################################################################
        query_token = clip.tokenize(query).to(self.device)
        query_feature = self.clip_model.encode_text(query_token)

        sim = get_similarity_no_loop(query_feature, self.images_features)

        top_indices = list(torch.Tensor.topk(sim, k=k, dim=-1)[1].squeeze())
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        self.model = nn.Sequential(
          nn.Linear(in_features=inp_dim, out_features=600),
          nn.Linear(in_features=600, out_features=num_classes)
        ).to(device)
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=6e-5, weight_decay=5e-4)
