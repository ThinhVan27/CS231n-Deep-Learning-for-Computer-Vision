...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (2)
        caption_in_embedding = word_embedding_forward(captions_in, W_embed)
        caption_out_embeding = word_embedding_forward(captions_out, W_embed)
        # (3)
        if self.cell_type == 'rnn':
          hidden_states = rnn_forward(caption_in_embedding, h0, Wx, Wh, b)
        else:
          pass
        # (4)
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        # Initialize h0
        h_prev = affine_forward(features, W_proj, b_proj) # NxH
        x0 = torch.squeeze(word_embedding_forward(torch.full(size=(N, 1), fill_value=self._start), W_embed))
        t = 0
        captions = []
        while t < max_length:
          h = rnn_step_forward(x0, h_prev, Wx, Wh, b)
          h_prev = h
          score = affine_forward(h, W_vocab, b_vocab)
          x0 = torch.argmax(score, dim=-1)
          captions.append(x0)
          x0 = torch.squeeze(word_embedding_forward(x0.unsqueeze(1), W_embed))
          t += 1
